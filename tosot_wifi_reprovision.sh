...
reconnect_to_fallback_wifi() {
    ...
    echo ">>> 🔁 Disconnecting before reconnect..."
    nmcli dev disconnect "$iface" >/dev/null 2>&1 || true
    sleep 2

    echo ">>> 🔁 Reconnecting to fallback WiFi: $fallback_ssid"
    nmcli -w 30 dev wifi connect "$fallback_ssid" ifname "$iface" >/dev/null 2>&1 && return 0
    nmcli -w 30 dev wifi connect "$fallback_ssid" >/dev/null 2>&1 && return 0
...
connect_to_ap() {
    ...
    if nmcli -t -f NAME connection show 2>/dev/null | grep -Fx "$ssid" >/dev/null; then
        ...
        nmcli con modify "$ssid" wifi-sec.key-mgmt wpa-psk wifi-sec.psk "$ap_password" >/dev/null 2>&1 || true
